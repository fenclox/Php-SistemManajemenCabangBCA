    ...
    if(in_array($imgExt, $valid_extensions)){
      if(move_uploaded_file($tmp, $path)){ // Cek apakah gambar berhasil diupload atau tidak
          ...
          if($sql1){ // Cek jika proses simpan ke database sukses atau tidak
              // Jika Sukses, Lakukan :
              header("location: ../../index.php?hal=tspv&tmb=success"); // Redirect ke halaman index.php
          }
      } else {
        ...
        // Jika Gagal, Lakukan :
        header("location: ../../index.php?hal=tspv&tmb=fail");
    }
}
//Proses Ubah
    ...
    if(isset($_POST['ubah_foto'])){ // Jika user menceklis checkbox yang ada di form ubah, lakukan :
        ...
        if(move_uploaded_file($tmp, $path)){ // Cek apakah gambar berhasil diupload atau tidak
            ...
            $query = "UPDATE pegawai SET nm_pegawai='".$nm."', jenis_kelamin='".$jk."', tempat_lahir='".$tmp_lhr."', tanggal_lahir='".$tgl_lhr."', agama='".$agm."',alamat='".$alm."', no_telp='".$telp."', tahun_masuk='".$thn_msk."', bagian=Supervisor, kd_bank='".$cbg."', password='".$pass."', foto='".$fotobaru."' WHERE nip='".$nip."'";
            $sql = mysql_query($query); // Eksekusi/ Jalankan query dari variabel $query

            if($sql){ // Cek jika proses simpan ke database sukses atau tidak
                // Jika Sukses, Lakukan :
                header("location: ../../index.php?hal=spv&ubh=success"); // Redirect ke halaman
            }else{
                // Jika Gagal, Lakukan :
...
//Proses Hapus
    ...
    $sql1 = mysql_query($query1);
        ...
        if ($sql1) {
            header("Location: ../../index.php?hal=spv&hps=success");
        } else {
            header("Location: ../../index.php?hal=spv&hps=fail");
        }
         exit;
}
?>
